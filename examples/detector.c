...
void validate_detector_recall_percls(char *cfgfile, char *weightfile)
{
    network *net = load_network(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    fprintf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net->learning_rate, net->momentum, net->decay);
 ...
 fprintf(stderr, "%5d\tRecall:%.2f%%\n", j, 100.0*correct/total);
...
void print_detections(char *filename,box *boxes, float **probs,float thresh, int total, int classes, int w, int h)
{
   ...
   int class = max_index(probs[i], classes);
        ...
        float ymax = (boxes[i].y + boxes[i].h/2.0)*h;
...
void test_detectord(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
   list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
...
//list *plist = get_paths("imlist.txt"); 
list *plist = get_paths(filename);
int N = plist->size;
printf("There are %d files in the given list file.\n",N);
char **paths = (char **)list_to_array(plist);
    ...
    for(i = 0; i < N; ++i){
        ...
        draw_detections(im, l.w*l.h*l.n, thresh, boxes, probs, masks, names, alphabet, l.classes);
	print_detections(input,boxes, probs,hier_thresh, l.w*l.h*l.n, l.classes, im.w, im.h);
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "testd")) test_detectord(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile, fullscreen);
